...
# -*- coding: utf-8 -*-
"""
SSH Tunnel Proxy — رابط گرافیکی PyQt5 برای مدیریت تونل SOCKS با autossh.
...
APP_NAME = "SSH Tunnel Proxy"
APP_SLUG = "ssh-tunnel-proxy"
APP_VERSION = "1.2.2"
CONFIG_DIR = Path.home() / ".config" / APP_SLUG
# پوشه‌های نام‌های قدیمی برای مهاجرت خودکار (جدیدترین اول)
_OLD_CONFIG_DIRS = [
    Path.home() / ".config" / "ssh-tunnel-manager",
    Path.home() / ".config" / "autossh-manager",
]
# مهاجرت خودکار از نام قدیمی تا داده‌های کاربر (سرورها/تنظیمات) از دست نرود
try:
    if not CONFIG_DIR.exists():
        for _old in _OLD_CONFIG_DIRS:
            if _old.exists():
                _old.rename(CONFIG_DIR)
                break
except Exception:
    pass
...
def set_system_socks(port):
    ...
    if gsettings_available():
        try:
            ...
            # خالی‌کردن خودکار HTTP/HTTPS/FTP سیستم تا در حالت manual فقط
            # SOCKS فعال بماند و پراکسی‌های قبلیِ کاربر تداخل ایجاد نکنند
            for schema in ("org.gnome.system.proxy.http",
                           "org.gnome.system.proxy.https",
                           "org.gnome.system.proxy.ftp"):
                try:
                    _gsettings_set(schema, "host", "")
                    _gsettings_set(schema, "port", "0")
                except Exception:
                    pass
            ...
            return True, f"System SOCKS set to 127.0.0.1:{port} (HTTP/HTTPS/FTP cleared)."
        except Exception as e:
            return False, str(e)
        ...
        try:
            ...
            for key, val in (("host", ""), ("port", "0")):
                try:
                    ...
                    pass
            # HTTP/HTTPS/FTP را هم تمیز نگه دار
            for schema in ("org.gnome.system.proxy.http",
                           "org.gnome.system.proxy.https",
                           "org.gnome.system.proxy.ftp"):
                try:
...
RC_MARK_BEGIN = "# >>> SSH Tunnel Proxy proxy >>>"
RC_MARK_END = "# <<< SSH Tunnel Proxy proxy <<<"
# نشانه‌های نام‌های قدیمی تا بلوک‌های پیشین هم شناخته و حذف شوند
_RC_MARK_BEGINS = [RC_MARK_BEGIN, "# >>> SSH Tunnel Manager proxy >>>"]
_RC_MARK_ENDS = [RC_MARK_END, "# <<< SSH Tunnel Manager proxy <<<"]
...
def write_terminal_proxy(port):
    ...
    try:
        ...
        body = (
            "# auto-generated by SSH Tunnel Proxy — do not edit\n"
...
def clear_terminal_proxy():
    ...
    try:
        CONFIG_DIR.mkdir(parents=True, exist_ok=True)
        body = (
            "# auto-generated by SSH Tunnel Proxy — tunnel is OFF\n"
...
def _strip_managed_block(text):
    ...
    نشانه‌های نام‌های قدیمی هم شناخته می‌شوند تا تکرار/باقی‌ماندن رخ ندهد."""
    lines = text.splitlines()
    out = []
    skip = False
    for ln in lines:
        if ln.strip() in _RC_MARK_BEGINS:
            skip = True
            continue
        if ln.strip() in _RC_MARK_ENDS:
            skip = False
            continue
        if not skip:
            out.append(ln)
...
def remove_rc_hook():
    ...
    try:
        ...
        for name in _RC_CANDIDATES:
            ...
            if any(m in old for m in _RC_MARK_BEGINS):
...
def rc_hook_installed():
    ...
    for name in _RC_CANDIDATES:
        rc = home / name
        try:
            if rc.exists() and any(m in rc.read_text("utf-8") for m in _RC_MARK_BEGINS):
                return True
        except Exception:
            pass
    return False
...
_OLD_AUTOSTART_FILES = [
    AUTOSTART_DIR / "ssh-tunnel-manager.desktop",
    AUTOSTART_DIR / "autossh-manager.desktop",
]
...
def autostart_enabled():
    if OS == "linux":
        return AUTOSTART_FILE.exists() or any(f.exists() for f in _OLD_AUTOSTART_FILES)
    if OS == "windows":
        try:
            import winreg
            key = winreg.OpenKey(
    ...
    try:
        if OS == "linux":
            if enable:
                ...
                # حذف فایل‌های با نام‌های قدیمی اگر باقی مانده باشند
                for old in _OLD_AUTOSTART_FILES:
                    if old.exists():
                        try:
                            old.unlink()
                        except Exception:
                ...
                for f in [AUTOSTART_FILE, *_OLD_AUTOSTART_FILES]:
                    if f.exists():
                        try:
                            f.unlink()
                        except Exception:
